...
import type {
  ...
  GlossaryTerm,
  KnowledgeSystemCard as CardType,
  TreeNode,
} from "../../../types/domain";

interface KnowledgeDimensionTabProps {
  systemId: string;
  treeData: TreeNode[];
...
export function KnowledgeDimensionTab({ systemId, treeData, dimensions, onSaved, isCreator = true }: KnowledgeDimensionTabProps) {
  ...
  const [editingDimension, setEditingDimension] = useState<DimensionItem | null>(null);
  const [previewItem, setPreviewItem] = useState<DimensionItem | null>(null);
  const [dimensionForm] = Form.useForm<Partial<DimensionItem>>();
  const [saving, setSaving] = useState(false);
  const [messageApi, contextHolder] = message.useMessage();
  const [dimensionLinks, setDimensionLinks] = useState<DimensionKnowledgeLink[]>([]);
  const [pendingDimensionSyncIds, setPendingDimensionSyncIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (!systemId) return;
    void domainApi.getDimensionKnowledgeLinks(systemId).then(setDimensionLinks);
    void domainApi.getPendingDimensionSync().then(setPendingDimensionSyncIds);
    void domainApi.getKnowledgeSystems().then(setKnowledgeSystems);
  }, [systemId]);

  const linkByDimensionId = useMemo(() => {
    const m: Record<string, DimensionKnowledgeLink> = {};
    dimensionLinks.forEach((l) => (m[l.dimensionId] = l));
    return m;
  }, [dimensionLinks]);

  const filtered = useMemo(
    () =>
      dimensions.filter((item) => (keyword ? item.name.includes(keyword) || item.code.includes(keyword) : true)),
    [dimensions, keyword],
  );
  ...
  const saveTree = useCallback(
    async (nextTree: TreeNode[]) => {
      try {
        ...
        await domainApi.updateKnowledgeDimensionTree(systemId, nextTree, session?.userId);
        const operator = session?.displayName ?? session?.username ?? "未登录";
        domainApi.appendOperationLog({
          module: "knowledge",
          moduleName: "语义知识库",
          actionType: "编辑",
          actionSummary: "编辑维度目录树",
          relatedObject: undefined,
          operator,
          operatorId: session?.userId,
          status: "成功",
        });
        messageApi.success("维度目录树已更新");
        onSaved?.();
      } catch (e) {
  ...
  const submitTreeNode = async () => {
    ...
    const newKey = `dim-node-${Date.now()}`;
  ...
  const openAddDimension = () => {
    ...
    dimensionForm.setFieldsValue({ name: "", code: "", parentName: "-", valueConstraint: "" });
  ...
  const openEditDimension = (item: DimensionItem) => {
    setEditingDimension(item);
    dimensionForm.setFieldsValue({
      name: item.name,
      code: item.code,
      parentName: item.parentName,
      valueConstraint: item.valueConstraint,
    });
    const link = linkByDimensionId[item.id];
    if (link) {
  ...
  const submitDimension = async () => {
    ...
    const parentName = (values.parentName as string) ?? "-";
    const valueConstraint = (values.valueConstraint as string) ?? "";
    ...
    try {
      if (editingDimension) {
        await domainApi.updateDimension(systemId, editingDimension.id, { name, code, parentName, valueConstraint }, session?.userId);
        await domainApi.setDimensionKnowledgeLink(systemId, editingDimension.id, { knowledgeSystemId, termIds });
        domainApi.appendOperationLog({
          module: "knowledge",
          moduleName: "语义知识库",
          actionType: "编辑",
          actionSummary: "编辑维度",
          relatedObject: editingDimension.name,
          operator,
          operatorId: session?.userId,
          status: "成功",
        });
        ...
        messageApi.success("维度已更新");
      } else {
        const list = await domainApi.createDimension(systemId, {
          name,
          code,
          parentName,
          valueConstraint,
        ...
        if (newId) {
          await domainApi.setDimensionKnowledgeLink(systemId, newId, { knowledgeSystemId, termIds });
        }
        domainApi.appendOperationLog({
          module: "knowledge",
          moduleName: "语义知识库",
  ...
  const clearPendingSync = async () => {
    try {
      await domainApi.clearPendingDimensionSync();
      setPendingDimensionSyncIds([]);
  ...
  return (
    <>
      {contextHolder}
      {pendingDimensionSyncIds.length > 0 && (
        <Alert
          type="info"
          message="以下维度关联的语义知识已变更，请核对"
          description={
            <Space>
              <span>涉及维度 ID：{pendingDimensionSyncIds.slice(0, 5).join("、")}{pendingDimensionSyncIds.length > 5 ? " 等" : ""}</span>
          ...
          <Card
            ...
            extra={
              <Space wrap>
                <Input.Search
                  allowClear
                  placeholder="按维度名称或编码搜索"
                  value={keyword}
                  onChange={(e) => setKeyword(e.target.value)}
                  style={{ width: 260 }}
                />
                {isCreator ? (
                  <Button type="primary" icon={<PlusOutlined />} onClick={openAddDimension}>
                    新增维度
                  </Button>
                ) : null}
              </Space>
                  ...
                  <Col xs={24} md={12} xl={8} key={item.id}>
                    <Card
                      ...
                      extra={
                        <Space>
                          {isCreator ? (
                            <Button type="link" size="small" icon={<EditOutlined />} onClick={() => openEditDimension(item)}>
                              编辑
                            </Button>
                          ) : null}
                          <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => setPreviewItem(item)}>
                            预览
                          </Button>
                        </Space>
                      }
      ...
      >
        <Form form={dimensionForm} layout="vertical">
          ...
          <Form.Item name="code" label="编码" rules={[{ required: true, message: "请输入编码" }]}>
            <Input placeholder="如 province" />
          </Form.Item>
          <Form.Item name="parentName" label="父维度">
            <Input placeholder="无则填 -" />
          </Form.Item>
          <Form.Item name="valueConstraint" label="维值约束">
            <Input.TextArea rows={2} placeholder="取值说明或约束" />
          </Form.Item>
        </Form>
      ...
      >
        ...
        <Typography.Paragraph>
          <strong>父维度：</strong>
          {previewItem?.parentName}
        </Typography.Paragraph>
        <Typography.Paragraph>
          <strong>维值约束：</strong>
          {previewItem?.valueConstraint}
        </Typography.Paragraph>
      </Modal>
    </>
  );
